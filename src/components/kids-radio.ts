...
export class KidsRadio extends KidsElement {
  ...
  private _outer: HTMLElement | null = null;
  ...
  protected onEnter(): void {
    ...
    if (this.boolAttr("checked")) {
      const inner = this.root.querySelector<HTMLElement>(".inner");
  ...
  private _select(): void {
    ...
    if (name) {
      ...
      if (parent) {
        const siblings = parent.querySelectorAll<KidsRadio>(`kids-radio[name="${name}"]`);
