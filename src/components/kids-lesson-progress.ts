...
export class KidsLessonProgress extends KidsElement {
  ...
  protected template(): string {
    ...
    let lessons: { title: string; status: string }[] = [];
    try {
  ...
  protected onEnter(): void {
    ...
    items.forEach((item, i) => {
      this.motionAnimate(item as Element, { opacity: [0, 1], x: [-15, 0] }, {
        type: "spring",
        stiffness: 300,
        damping: 22,
        delay: i * 0.06,
  ...
  private _bindEvents(): void {
    this.root.querySelectorAll(".lesson:not(.locked)").forEach((lesson) => {
      lesson.addEventListener("click", () => {
        const index = parseInt((lesson as HTMLElement).dataset.index || "0", 10);
